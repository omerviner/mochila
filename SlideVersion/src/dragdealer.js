...
Dragdealer.prototype = {
  ...
  renderHandlePosition: function() {
    ...
    if (this.options.css3 && StylePrefix.transform) {
	  // get image width size by pixels;
	  // find the right spot by persentage
	  // multiply the size by the persentage
	  // create image with overlapping areas by the start and the end
	  
	  var imgWidth = $(".handle").css('width').replace(/\D/g, '');
	  
	  var percentStartLoop = 0.9;
	  var percentEndLoop = 0.777;
	  
	  var startPixel = imgWidth * percentStartLoop;
	  var endPixel = imgWidth * percentEndLoop;
	  
	  
	  if (this.offset.current[0] < -endPixel){
